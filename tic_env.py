...
class TictactoeEnv:
    ...
    def step(self, position, print_grid=False):
        ...
        if self.grid[position] != 0:
            # changing it for DQN, game end when same position taken twice
            self.end = True
...
class Q_learningPlayer:
    '''
    Description:
        A class to implement an epsilon-policy Q-learning player with epsilon decay in Tic-tac-toe.

    Parameters:
        epsilon: float, in [0, 1]. This is a value between 0-1 that indicates the
            probability of making a random action instead of the optimal action
            at any given time.
        Q: Q-values dictionary storing states and the corresponding values
        lr: learning rate
        gamma: discount factor

    '''
    def __init__(self, epsilon, lr, gamma, player='X'):
        self.player = player # 'x' or 'O'
        self.epsilon = epsilon
        self.lr = lr
        self.gamma = gamma
        self.nA = 9       # Number of actions in the environment
        self.Q = defaultdict(lambda: np.zeros(9))    #  A nested dictionary that maps state -> (action -> action-value). 
        self.states = []
    ...
    def act(self, grid):
        '''decise the next step'''
        empty_position = np.array(self.empty(grid))
        state = str(grid)
        
        if random.random() < self.epsilon:
            # Random choose.
            action = empty_position[random.randint(0,len(empty_position)-1)]
        
        # Take a step
        else:                    
            # Greedy choose.
            values = np.array([self.Q[state][action] for action in empty_position])
            ...
            if len(ix_max) > 1:
                ...
                ix_select = ix_max[0]
            action = empty_position[ix_select]
        
        return (int(action / 3), action % 3)
    
    def update(self, pre_state, next_state, move, grid, reward, last=False):
        '''upgrade the Q-values'''
        action = 3 * move[0] + move[1]
        empty_position = np.array(self.empty(grid))
        values = np.array([self.Q[next_state][action] for action in empty_position])  # check available positions
        
        if len(values) >0 and last==False:
            self.Q[pre_state][action] += self.lr * (reward + self.gamma * np.max(values) - self.Q[pre_state][action])
        
        else:
            self.Q[pre_state][action] += self.lr * (reward - self.Q[pre_state][action])
        
    def eps_decay(self, epoch, n_star, min_e=0.1, max_e=0.8):   # update epsilon
        self.epsilon = max(min_e, max_e*(1-epoch/n_star))
